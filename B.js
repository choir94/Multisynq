...
// Function to send EVM transactions from Sepolia to Babylon or Holesky
async function sendEvmTransaction(walletInfo, maxTransaction, destination, telegramBot = null, chatId = null) {
  const wallet = new ethers.Wallet(walletInfo.privateKey, provider());
  let recipientAddress, destinationName, channelId, operand;
  if (destination === "babylon") {
  ...
  // Debug operand
  logger.info(`Operand length: ${operand.length}`);
  logger.info(`Operand: ${operand.slice(0, 100)}...${operand.slice(-100)}`); // Log first and last 100 chars

  // Validate operand
  if (!operand.startsWith("0x") || !isValidHex(operand.slice(2))) {
    logger.error(`Invalid operand format: ${operand.slice(0, 100)}...`);
    return;
  }
  const expectedLength = destination === "babylon" ? 1232 : 1208;
  if (operand.length !== expectedLength) {
    logger.error(`Invalid operand length: ${operand.length}, expected ${expectedLength}`);
    return;
  }
